...
# ロバスト回帰の使用有無の入力
# -------------------------------
robust_flag = st.checkbox("ロバスト回帰を使用する", value=False)
        ...
        if len(x_data) != len(y_data) or len(x_data) == 0:
            ...
            if model_type == "全モデル比較":
                ...
                for name, setting in model_settings.items():
                    ...
                    try:
                        if robust_flag:
                            # ロバスト回帰によるパラメータ推定（HUBER loss）
                            def residuals_func(params, x, y, model):
                                return model(x, *params) - y
                            result = least_squares(residuals_func, p0, args=(x_data, y_data, func), bounds=bounds, loss='huber')
                            params = result.x
                        else:
                ...
                # スピアマンの順位相関係数を計算し、テキストデータとして表の下に表示（日本語）
            ...
            # -------------- 単一モデル選択の場合 --------------
                ...
                if robust_flag:
                    ...
                    params, pcov = curve_fit(func, x_data, y_data, p0=p0, bounds=bounds)
                a, b, c = params
                perr = np.sqrt(np.diag(params*0+1))  # robust版では標準誤差は得にくいためダミー表示
                y_fit = func(x_data, a, b, c)
                r2 = compute_r2(y_data, y_fit)
